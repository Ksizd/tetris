...
let glyphImagePromise: Promise<HTMLImageElement> | null = null;
let cachedGlyphImage: HTMLImageElement | null = null;

let mahjongTileTexture: THREE.CanvasTexture | THREE.DataTexture | null = null;
let mahjongTileTextureReady = false;
...
export function createMahjongTileTexture(size = DEFAULT_TILE_SIZE): THREE.Texture {
  if (typeof document === 'undefined') {
    ...
    mahjongTileTextureReady = true;
    mahjongTileTexture = placeholder;
    return placeholder;
  }
  ...
  drawAtlas(ctx, size, cachedGlyphImage);
  mahjongTileTextureReady = Boolean(cachedGlyphImage && cachedGlyphImage.complete && cachedGlyphImage.naturalWidth > 0);

  loadGlyphImage()
    .then((glyph) => {
      drawAtlas(ctx, size, glyph);
      texture.needsUpdate = true;
      mahjongTileTextureReady = true;
    })
    .catch(() => {
      /* ignore glyph load errors for now */
    });

  mahjongTileTexture = texture;
  return texture;
}
...
export function createCanonicalTileMaterials(atlasSize: number = DEFAULT_TILE_SIZE): TileMaterials {
  const tileTexture = getMahjongTileTexture(atlasSize);
  const effectiveSize =
...
function getGlyphImage(): HTMLImageElement {
  throw new Error('getGlyphImage is deprecated. Use loadGlyphImage instead.');
}

export function getMahjongTileTexture(size: number = DEFAULT_TILE_SIZE): THREE.Texture {
  if (mahjongTileTexture) {
    return mahjongTileTexture;
  }
  return createMahjongTileTexture(size);
}

function loadGlyphImage(): Promise<HTMLImageElement> {
  if (glyphImagePromise) {
    return glyphImagePromise;
  }
  if (cachedGlyphImage && cachedGlyphImage.complete && cachedGlyphImage.naturalWidth > 0) {
    return Promise.resolve(cachedGlyphImage);
  }
  const img = new Image();
  img.src = GLYPH_DATA_URL;

  glyphImagePromise = new Promise((resolve, reject) => {
    const finalize = (image: HTMLImageElement) => {
      cachedGlyphImage = image;
      resolve(image);
    };
